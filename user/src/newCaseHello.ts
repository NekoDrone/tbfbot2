...
const firestoreCollection = new firestore.Firestore({
  // put up here because of how functions are instantiated. improves response times.
  projectId: PROJ_ID,
...
async function userExistsinFirestore(teleUser: string) {
  const caseData = await firestoreCollection
    .where("telegramhandle", "==", teleUser)
    .get();
