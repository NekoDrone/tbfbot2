...
const AUTH_USERS_COLLECTION: string =
  process.env.AUTH_USERS_COLLECTION ?? "undefined";
export const firestoreObject = new firestore.Firestore({
  projectId: PROJ_ID,
});
export const firestoreCollection = firestoreObject.collection(
...
export const jiraCredentials = Buffer.from(
  JIRA_EMAIL + ":" + JIRA_KEY,
  "base64"
).toString();
